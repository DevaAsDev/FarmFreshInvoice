...
namespace FarmFreshInvoice
{
...

    {
        ...
        public Window1(string billId, string billTime, string billDate, int billNum)
        {
            InitializeComponent();
        ...
        private async Task GetDataFromFirebaseAsync()
        {
            try
            {
                ...
                var data = await handler.GetDataAsync("bill/");
        ...
        private void save_btn(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    var SetData = conn.client.Set("billCollection/user/retail/cash/" + billDateOf + "/" + billId, newBill);
                    conn.client.Update("Settings/billNumber/", todo);
                }
                else if (paymentMethod == "UPI")
                {
                    var SetData = conn.client.Set("billCollection/user/retail/upi/" + billDateOf + "/" + billId, newBill);
                    conn.client.Update("Settings/billNumber/", todo);
                }
                else
                {
                    var SetData = conn.client.Set("billCollection/user/retail/pending/" + billDateOf + "/" + billId, newBill);
                    conn.client.Update("Settings/billNumber/", todo);
                }
